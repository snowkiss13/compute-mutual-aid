...
| P | 項目 | 目的 | 担当 | 状態 |
|---|------|------|------|------|
| 1 | rate-limit | ジョブspam DoS防止（初期0でもRedis/queue枯渇） | Codex | **委譲中** |
| 2 | arbitrage防御 | echo/ollama由来creditをclaude等高コストへ移転不可（model/provider別bucket） | Codex | 設計済・待機 |
| 3 | OpenAI互換ゲートウェイ | `/v1/chat/completions`で既存SDK/ツールから直叩き→採用障壁↓ | Codex | 設計待ち |
| 4 | MCPサーバ公開 | エージェントがネイティブに発見・利用できる「正しい形」 | Codex | 設計待ち |
