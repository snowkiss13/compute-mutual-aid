...
> ## ⚠️ セキュリティ状態（2026-05-30）
> - **実行安全（tool-less・認証情報不通過）は成立・堅牢。** これが本システムの中核保証。
> - **身元/クレジット保証は未成立（P5実装中）。** 現状の認証は共有トークン1個のみで、`account` は呼び出し側が自由に名乗れる文字列。
>   → claude allowlist・クレジット帰属・レート制限は「認証されていない account」をキーにしており、現時点では実質的な保護になっていない。
> - **したがって: トークンの共有・Lolipop manifest の公開・claude provider の起動は、P5（per-account APIキーによる身元束縛）が入るまで行わないこと。**
> - 現時点はトークン未共有・manifest未公開・claude provider不在のため実害なし。
